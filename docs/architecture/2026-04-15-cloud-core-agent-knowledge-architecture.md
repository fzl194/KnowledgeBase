...
| 层级 | 中文名称 | 核心含义 | 默认是否检索 |
| --- | --- | --- | --- |
| L0 | 原始语料层 | 从 source artifact 解析出来的原始段落，保留来源、章节、原文、block 形态和通用 scope/facet | 否 |
| L1 | 归并语料层 | 对 L0 原始段落去重、聚类、归并后形成的检索主对象 | 是 |
| L2 | 来源映射与差异层 | 记录 L1 由哪些 L0 构成，以及是否存在 scope、来源、版本或内容差异 | 否 |
...
PDF、Word、ZIP、多厂家复杂版式解析后置，但 schema 必须能记录这些原始来源。M1 当前实现可以先消费上游处理好的 Markdown；当原始语料来自产品 HTML 包时，建议链路为：

```text
HDX/HWICS
  -> productdoc_to_md.py
  -> Markdown 输出目录 + html_to_md_mapping.json
  -> Knowledge Mining
```

Mining 必须把原始 HTML/PDF/DOC 等来源路径、转换后 Markdown 路径、转换质量和 topic mapping 信息写入 `raw_documents`，以便后续重新解析、溯源和质量审计。
...
原始语料可能来自产品 HTML、转换后 Markdown、PDF、DOC/DOCX、专家手写文档或项目交付文档。schema v0.4 使用以下原则：

- `raw_documents.file_type` 记录主输入格式：`markdown/html/pdf/doc/docx/txt/mixed/other`。
- `raw_documents.raw_storage_uri` 记录原始来源，例如 HTML/PDF/DOC。
- `raw_documents.normalized_storage_uri` 记录转换后 Markdown 或纯文本。
- `raw_documents.scope_json` 记录产品、版本、网元、项目、作者、环境、客户、专题等可选适用范围。
- `raw_documents.tags_json` 记录主题、场景和对象标签。
- `raw_segments.block_type` 记录结构形态：paragraph/list/table/html_table/code/raw_html/unknown。
- `raw_segments.section_role` 记录章节语义：parameter/example/procedure_step/troubleshooting_step/alarm_meaning/constraint 等。

对于 `productdoc_to_md.py` 生成的产物，Mining 应读取 `html_to_md_mapping.json`，至少保留 `topic_id`、`parent_id`、`topic_path`、`html_rel_path`、`md_rel_path`、`mode` 和 `child_count`。
...
- 只负责离线生产知识资产。
- 支持上游转换后的 Markdown 解析，生成 L0 原始语料层；对 `productdoc_to_md.py` 产物必须读取 html-to-md mapping 并保留原始 HTML 路径。
...
- 第一阶段聚焦命令与证据优先。
- 第一阶段 Mining 当前可优先消费上游转换后的 Markdown，但 schema 支持 Markdown/HTML/PDF/DOC/DOCX/TXT 等原始来源记录。
